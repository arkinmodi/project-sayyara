...
\begin{longtable}{p{0.2\textwidth} p{0.2\textwidth} p{0.5\textwidth}}
	...
	March 8, 2023     & Arkin Modi            & Add Traceability Between Test Cases and Requirements \& Traceability Between Test Cases and Modules Sections \\
	March 8, 2023     & Timothy Choy          & Add Test Evaluation for Usability Survey                                                                     \\
	March 8, 2023     & Arkin Modi            & Add Reflection                                                                                               \\
	March 8, 2023     & Joy Xiao              & Add Reflection                                                                                               \\
	March 8, 2023     & Leon So               & Add Reflection                                                                                               \\
	March 8, 2023     & Timothy Choy          & Add Reflection                                                                                               \\
	...
	April 1, 2023     & Leon So               & Update authentication tests due to update in SRS                                                             \\
	April 5, 2023     & Leon So               & Update changes due to testing                                                                                \\
	\bottomrule
\end{longtable}
...
adequate usability and to verify that the system is in a functional state for the end users. The
...
\begin{enumerate}
	...
	\item \textbf{FRT-BE1-4}

	      Control: Manual

	      Initial State: Customer with no existing account

	      Input: Customer submits sign up form with invalid email, password, name, and phone number

	      Output: Input validation messages should be shown indicating that provided inputs are invalid

	      Test Case Derivation: New customer wants to sign up for a new account with invalid sign up
	      information provided
	...
	\item \textbf{FRT-BE1-5}

	      Control: Manual

	      Initial State: Customer with no existing account

	      Input: Customer attempts to submit input validation form with empty inputs

	      Output: Input validation messages should be shown indicating that provided inputs are invalid

	      Test Case Derivation: New customer wants to sign up for a new account with empty fields provided

	      How test will be performed: The tester will register and sign up for a new customer account with
	      empty fields provided

	      Results: Passed

	\item \textbf{FRT-BE1-6}

	      Control: Manual

	      Initial State: Shop owner with no existing account

	      Input: Shop owner submits sign up form with invalid email, password, name, and phone number

	      Output: Input validation messages should be shown indicating that provided inputs are invalid

	      Test Case Derivation: New shop owner wants to sign up for a new account with invalid sign up
	      information provided
	...
	\item \textbf{FRT-BE1-7}

	      Control: Manual

	      Initial State: Shop owner with no existing account

	      Input: Shop owner attempts to submit input validation form with empty inputs

	      Output: Input validation messages should be shown indicating that provided inputs are invalid

	      Test Case Derivation: New shop owner wants to sign up for a new account with empty fields provided

	      How test will be performed: The tester will register and sign up for a new shop owner account with
	      empty fields provided

	      Results: Passed

	\item \textbf{FRT-BE1-8}

	      Control: Manual

	      Initial State: Employee with no existing account

	      Input: Employee submits sign up form with invalid email, password, name, and phone number

	      Output: Input validation messages should be shown indicating that provided inputs are invalid

	      Test Case Derivation: New employee wants to sign up for a new account with invalid sign up
	      information provided
	...
	\item \textbf{FRT-BE1-9}

	      Control: Manual

	      Initial State: Employee with no existing account

	      Input: Employee attempts to submit input validation form with empty inputs

	      Output: Input validation messages should be shown indicating that provided inputs are invalid

	      Test Case Derivation: New employee wants to sign up for a new account with empty fields provided

	      How test will be performed: The tester will register and sign up for a new employee account with
	      empty fields provided

	\item \textbf{FRT-BE1-10}

	      Control: Manual

	      Initial State: Employee with no existing account

	      Input: Employee submits sign up form with invalid shop ID

	      Output: Notification should appear indicating that the provided shop ID is invalid

	      Test Case Derivation: New employee wants to sign up for a new account with invalid shop ID provided
	...
	\item \textbf{FRT-BE2-4}

	      Control: Manual

	      Initial State: Customer with existing account and not logged in

	      Input: Customer submits login form with invalid email and password

	      Output: Notification should appear indicating that the login was unsuccessful

	      Test Case Derivation: Customer wants to log in to existing with invalid login credentials provided

	      How test will be performed: The tester will log in to an existing customer account using invalid
	      login credentials

	      Results: Passed

	\item \textbf{FRT-BE2-5}

	      Control: Manual

	      Initial State: Shop owner with existing account and not logged in

	      Input: Shop owner submits login form with invalid email and password

	      Output: Notification should appear indicating that the login was unsuccessful

	      Test Case Derivation: Shop owner wants to log in to existing with invalid login credentials
	      provided

	      How test will be performed: The tester will log in to an existing shop owner account using invalid
	      login credentials

	      Results: Passed

	\item \textbf{FRT-BE2-6}

	      Control: Manual

	      Initial State: Employee with existing account and not logged in

	      Input: Employee submits login form with invalid email and password

	      Output: Notification should appear indicating that the login was unsuccessful

	      Test Case Derivation: Employee wants to log in to existing with invalid login credentials provided

	      How test will be performed: The tester will log in to an existing employee account using invalid
	      login credentials
...
\begin{enumerate}
	...
	\item \textbf{FRT-BE5-2}
	      ...
	      Test Case Derivation: The shop owner/employee user is able to accept an appointment request
	...
	\item \textbf{FRT-BE5-3}
	      ...
	      Test Case Derivation: The shop owner/employee user is able to move a scheduled appointment to in
	      progress

	      How test will be performed: The tester will go through the user interface to press ``in progress''
	      on a scheduled appointment and confirm the appointment is moved to the in progress appointments tab
	...
	\item \textbf{FRT-BE5-4}
	      ...
	      Test Case Derivation: The shop owner/employee user is able to move an in Progress appointment to
	      completed
...
\begin{enumerate}
	...
	\item \textbf{NFRT-UH4-1}
	      ...
	      Results: Failed. There were some critical issues and bugs found that affected the usability of the
	      application. Users had trouble understanding some filters and were unable to recover from some
	      errors. The full details of the survey are in section \ref{Usability Survey Results}.
...
``NFRT-OE-1'' failed as some views and components did not scale as intended on some small and
medium device dimensions (e.g., iPhone SE). The CSS has been updated accordingly to support all
device dimensions listed in the
\href{https://github.com/arkinmodi/project-sayyara/blob/main/docs/SRS/SRS.pdf}{SRS}. ``NFRT-SR2-1''
failed because extra information was being returned from the API call that is not needed for these
endpoints, thereby exposing data that does not belong to the user. As a result, the endpoint has
been revised to return an appropriate set of information that does not expose more information than
needed and does not violate the security requirements. Additionally, there were comments made
regarding the lack of the ability to edit and cancel appointments in the user interface during the
Revision 0 Demonstration. This feedback will also be addressed. All changes are planned to be
completed before the Revision 1 Demonstration.
...
\begin{landscape}
	...
	\begin{longtable}{|l|cccccccccccccc|}
		\caption{Traceability Between Functional Test Cases and Functional Requirements, BE17 to BE30}                                                                                                                                                                                                  \\
		\hline
		\textbf{Test IDs}   & \multicolumn{14}{c|}{\textbf{Functional Requirement IDs}}                                                                                                                                                                                                                 \\
		\hline
		~                   & \textbf{BE17}                                             & \textbf{BE18} & \textbf{BE19} & \textbf{BE20} & \textbf{BE21} & \textbf{BE22} & \textbf{BE23} & \textbf{BE24} & \textbf{BE25} & \textbf{BE26} & \textbf{BE27} & \textbf{BE28} & \textbf{BE29} & \textbf{BE30} \\
		\hline
		\textbf{FRT-BE17-1} & X                                                         & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             \\
		\textbf{FRT-BE18-1} & ~                                                         & X             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             \\
		\textbf{FRT-BE18-2} & ~                                                         & X             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             & ~             \\
...
\subsection{Usability Survey Results} \label{Usability Survey Results}

The following are the results of the usability survey. Four people were interviewed as CUSTOMER
users, and several mechanics were interviewed as SHOP OWNER and EMPLOYEE users. The following were
the main points brought out through user testing

\begin{enumerate}
	\item Were there any bugs or issues you ran into?

	      \begin{itemize}
		      \item Screen resizing issues when changing screen window size
		      \item Scrolling while in a dropdown would move the dropdown unexpectedly
	      \end{itemize}

	\item What did you like the most or what stood out for you while using the application?

	      \begin{itemize}
		      \item Colour scheme is good
		      \item The website works in dark mode
	      \end{itemize}

	\item Please share what changes you'd like to see on our website.

	      \begin{itemize}
		      \item Change the multiselect for parts into radio buttons, as they are confusing
		      \item Customers may not know what OEM or AFTERMARKET stands for, clarification would be helpful
		      \item Make the logo at the top left more reactive (let the mouse turn into a clickable icon to tell the
		            user you can click on the logo)
		      \item The logo seems a bit unrefined. A more professional logo would be nice
		      \item The email address and phone numbers for each shop profile should allow customers to immediately
		            navigate to them (either an email or phone call)
		      \item There are few car companies to choose from
	      \end{itemize}

	\item Was the website intuitive, easy to learn to use and navigate?

	      \begin{itemize}
		      \item The website could be more clean, there are many steps need to reach tasks such as booking an
		            appointment
		      \item The route between booking a canned appointment and a custom appointment was slightly confusing,
		            there is no way to create a quote from the chat page
	      \end{itemize}

	\item When you made a mistake while navigating or using the application, was it easy to recover?

	      \begin{itemize}
		      \item The website crashed once, and I was given a critical warning where I had to refresh the page to
		            recover
	      \end{itemize}
...
\begin{enumerate}[\textbf{\arabic*.}]
	\item \textbf{In what ways was the Verification and Validation (VnV) Plan different from the activities that were
	      ...
	      There were some changes that we made due to changes in the requirements. There were some areas
	      where we decided was out of scope and communicated with our supervisor. In addition, during
	      implementation we found that some changes were necessary to achieve a desirable user flow and
	      experience. This resulted in some changes in the requirements and testing plan. Since we had not
	      anticipated these requirement changes, modifications were required to the VnV plan to appropriately
	      reflect the changes in requirements. In addition, we decided to create and include a usability
	      survey to gather feedback from users.

	      We would be able to anticipate most of these changes in future projects. Some changes that are
	      difficult to anticipate are stakeholders changing the requirements which would require updating the
	      requirements documentation and consequently the testing plans. With regards to the usability
	      survey, we would likely consider including a usability survey earlier in the process, and therefore
	      we would likely be able to anticipate this component of the VnV plan in the future.
\end{enumerate}
