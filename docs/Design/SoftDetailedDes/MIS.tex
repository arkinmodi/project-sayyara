...
\begin{itemize}
	\item output: A list of Shops from the database table, such that the Shops are filtered by each of the
	      ...
	      $location := (b \in \text{Shops} | \text{distance} <= distance(b.lat, b.long, lat, long))$ \\
	      $services := (c \in \text{Shops} | \text{services} \in \text{c.Services})$ \\ $parts := (d \in
		      \text{Shops} | \text{parts} \in \text{d.Parts})$ \\ $out := (s \in (a \land b \land c \land d))$
\end{itemize}
...
distance(shop_lat, shop_long, user_lat, user_long) $\equiv (3959 \cdot
	\arccos(\cos(\text{shop_lat})) \cdot \cos(\text{user_lat}) + \cos(\text{shop_long} -
		\text{user_long}) + \sin(\text{shop_lat}) \cdot \sin(\text{user_lat}))$
