...
\begin{table}[H]
	...
	\begin{tabular}{|p{0.3\textwidth}|p{0.6\textwidth}|}
		...
		\hline
		SR2                  & Separate views are created for each type of user, so that they have access to only information that the specific user needs. Each account is created with a unique user type, so that they can only access the correct view. \\
	\end{tabular}
\end{table}
